...
class PathClass:
  ...
  def PotentialAction(self,slice1,slice2):
     ...
     return 0.5*self.tau*Vexternal # (Vinteraction+Vexternal)
...
def PIMC(numSteps,Path,myMove):
   ...
   for steps in range(0,numSteps):
         ...
         (accepted,attempted)=DisplaceMove(Path,accepted,attempted)
         if steps % observableSkip==0 and steps>500:
...
#   print "PTCL TO MOVE",ptclToMove                                                  
   ...
   oldKA=0.0
   for slice in range(1,maxStepSize):
     disp=Path.beads[slice,ptclToMove]-Path.beads[slice+1,ptclToMove]
     dist2=numpy.dot(disp,disp)
     oldKA=oldKA-Path.KineticAction(slice-1,slice)
     oldKA=oldKA-Path.PotentialAction(slice-1,slice)
   ...
   for slice in range(1,maxStepSize):
      disp=Path.beads[slice,ptclToMove]-Path.beads[slice+1,ptclToMove]
      dist2=numpy.dot(disp,disp)
      newKA=newKA-Path.KineticAction(slice-1,slice)
      newKA=newKA-Path.PotentialAction(slice-1,slice) #Path.V_external(slice)+Path.V_internal(slice)                                                   
   if not((newKA-oldKA-(T_newToOld-T_oldToNew))>numpy.log(random.random())):
     Path.beads[0:maxStepSize,ptclToMove]=oldCoords.copy()
   else:
...
def DisplaceMove(Path,accepted,attempted):
   attempted=attempted+1
   Path.RelabelBeads()
   ptclToMove=random.randint(0,1)
   oldCoords=Path.beads[0:Path.NumTimeSlices,ptclToMove].copy()
   oldKA=0.0
   for slice in range(0,Path.NumTimeSlices-1):
     disp=Path.beads[slice,ptclToMove]-Path.beads[slice+1,ptclToMove]
     dist2=numpy.dot(disp,disp)
     oldKA=oldKA-Path.PotentialAction(slice,slice+1)
   oldKA=oldKA-Path.PotentialAction(Path.NumTimeSlices-1,0)
   Path.beads[:,ptclToMove]=Path.beads[:,ptclToMove]+numpy.array([random.random()-0.5,random.random()-0.5,random.random()-0.5])
   newKA=0.0
   for slice in range(0,Path.NumTimeSlices-1):
      disp=Path.beads[slice,ptclToMove]-Path.beads[slice+1,ptclToMove]
      dist2=numpy.dot(disp,disp)
      newKA=newKA-Path.PotentialAction(slice,slice+1) #Path.V_external(slice)+Path.V_internal(slice)                                                   
   newKA=newKA-Path.PotentialAction(Path.NumTimeSlices-1,0) #Path.V_external(slice)+Path.V_internal(slice)                                                   
   if not((newKA-oldKA)>numpy.log(random.random())):
     Path.beads[0:Path.NumTimeSlices,ptclToMove]=oldCoords.copy()
   else:
     accepted=accepted+1
   return (accepted,attempted)
